...
const extractInterface = path => path
  .split('/')
...
application.createSandbox = (context = {}) => {
  ...
  return okra.vm.createContext(context);
};
...
application.setRPCMethod = (
  ...
  app.rpcApi[interfaceName][methodName] = fn(api);
};
  ...
  try {
    ...
    interfaces.forEach(
      interfaceName => application
        .loadRPCMethods(app, rpcDirectory, interfaceName)
    );
  } catch (error) {
    api.log.error(error);
  }
...
application.watchFileSystem = () => {
  ...
  const resetMethod = application.setRPCMethod.bind(null, app);

  const apiDirectory = okra.config.application.directory + '/api/http/';
  const rpcDirectory = okra.config.application.directory + '/api/jstp/';

  const existsHttp = okra.fs.existsSync(apiDirectory);
  const existsRpc = okra.fs.existsSync(rpcDirectory);
...
application.watchRPCDirectory = (
  ...
  watcher.watch();
  watcher.on('change', (path, file) => {
    const interfaceName = extractInterface(path);
    resetMethod(interfaceName, path, file);
  });
};
